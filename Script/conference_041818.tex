...
\author{\IEEEauthorblockN{1\textsuperscript{st} Chen Wang}
...
\begin{abstract}
The use of millimeter wave frequency bands for transmission can improve data rate with the help of beamforming technology to overcome the high path and penetration losses.
However for vehicle, when high resolution beam codebook are used, the high mobility of vehicle results in extremely frequent beam alignment and significant overhead.
...
Simulation results show that proposed LSTM-based method outpreforms EKF based method, and can prevent frequent beam alignment to reduce overhead while ensuring acceptable signal-to-noise ratio (SNR).
\end{abstract}

\begin{IEEEkeywords}
beam tracking, mmWave vehicular networks, LSTM, extended Kalman filter
\end{IEEEkeywords}
...
% 不同于传统全向天线的信道，阵列天线的信道具有极强的方向性 *，这意味着若想形成较高的波束增益，必须保证接收端和发送端的波束之间相互对准，否则波束对的失准将会带来严重的增益下降，降低通信性能 *。
...
% 相较于低速的用户终端，为高速车辆提供可靠的波束对准更具挑战性，因为较高的速度将会导致车辆更快驶出波束的覆盖范围，为了保证车辆始终处于较高的波束增益范围，需要进行高频率的对准，而高频的波束会进一步加大系统的开支，尤其是当采用高分辨率码本的时候。
% 幸运的是，由于毫米波信道的稀疏性 *，毫米波信道的 AOA、AOD 与收发端位置的具有一定的相关性 *，在 LOS 信号下沿固定道路行驶的车辆的信道信息应该能被很好的预测出来。
...
% 转置，共轭转置
$({\dot})^{T}$ denote transpose and  $(\bullet)^H$ denote conjugate transpose (Hermitian).
% 矩阵指定行、列、元素
...
% 2 范数
Beside, $\Vert_pair{\bullet}_2$ denote $\mathcal{l}_2$-norm of a vector.
% 复数、实数集合
