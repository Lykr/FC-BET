...
### Review 3
- [x] 图 2 描述不清晰：The description of the proposed methodology is left wanting for clarity. For instance, the scheme is apparently described in Fig. 2, but is not accompanied by relevant details in the text. Reset initial estimation, reset feature, and many other aspects of the scheme require better explanation.
...
- [x] 没有解释标题的快和连续：The title mentions the keywords "Fast" and "Consecutive Beam Tracking Scheme" in the title. However, it does not explicitly say anywhere in the text why the scheme is "Fast" and why it has the qualifier "Consecutive Beam Tracking"
...
### Review 4
- [ ] 0.7 到 0.9 的中断率过高：In figure 7, the outage performance takes from 0.7 to 0.9. If we consider the operation of wireless communication systems, the evaluated outage is too worse. The authors should evaluate the lower outage performance.