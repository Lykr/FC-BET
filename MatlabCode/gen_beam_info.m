function beam_info = gen_beam_info(antenna_num, beam_num)
    ...
    % hierarchical book, 2^n, 0 - 1, 1 - 2, 2 - 4, ...
    level = log2(antenna_num) + 1;
    beam_book_h = cell(level, 1);
    beam_angles_h = cell(level, 1);
    
    for i = 1 : level
        beam_num_h = 2 ^ (i - 1);
        beam_angles_h_i = [pi / beam_num_h / 2 : pi / beam_num_h : pi];
        beam_angles_h{i} = beam_angles_h_i;
        beam_book_h{i} = zeros(antenna_num, beam_num_h);
        for j = 1 : beam_num_h
            beam_book_h{i}(:, j) = [gen_eMatrix(beam_num_h, beam_angles_h_i(j)); zeros(antenna_num - beam_num_h, 1)];
        end
    end
    beam_info.beam_book_h = beam_book_h;
    beam_info.beam_angles_h = beam_angles_h;
end
