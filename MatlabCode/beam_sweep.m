...
noise_t = (randn(veh_beam_num, bs_beam_num, n_r) + 1i * randn(veh_beam_num, bs_beam_num, n_r)) .* sqrt(var_n / 2); % noise part of received signal
a = i;
b = j;

N_t = zeros(veh_beam_num, bs_beam_num);
for i = 1 : veh_beam_num
    for j = 1 : bs_beam_num
        N_t(i, j) = veh_beam_book(:, i)' * reshape(noise_t(i, j, :), n_r, 1);
    end
end
N_t(best_veh_beam, best_bs_beam) = N(a, b);
CSI.noise_h = N_t;
